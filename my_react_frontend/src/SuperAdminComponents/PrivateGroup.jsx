...
import { privateGroupIconClickedContext } from "../App";
import WriteMessage from "./WriteMessage";
import Invite from "./Invite";
export const privateMessageContext = createContext(undefined);
export const invitePrivateButtonClickedContext = createContext(undefined);

function PrivateGroup(props) {
    const user = useContext(userContext)[0];
    const containerRef = useRef();
    const clickOnPrivateItem = useContext(clickOnPrivateItemContext)[0];
    const setPrivateGroupIconClicked = useContext(privateGroupIconClickedContext)[1];
    const [message, setMessage] = useState("");
    const [allMessages, setAllMessages] = useState([]);
    const setFeedBackMessage = useState([])[0];
    const [invitePrivateButtonClicked, setInvitePrivateButtonClicked] = useState(false);
    const [users, setUsers] = useState([]);
    const [currentPdf, setCurrentPdf] = useState(null);
    const handleClick = () => {
        setPrivateGroupIconClicked(true);
    }
    ...
    dataToBackend.append('id', clickOnPrivateItem);
    useEffect(() => {
    ...
    const handleMessageSubmit = (e) => {
        ...
        dataToBackend.append('groupId', parseInt(clickOnPrivateItem));
    ...
    let foundGroup = user.role === 1 ? props.privateGroups.find(group => group.id === clickOnPrivateItem)
        : props.privateGroups.find(group => group['group'].id === clickOnPrivateItem);
    ...
    return (<invitePrivateButtonClickedContext.Provider value={[invitePrivateButtonClicked, setInvitePrivateButtonClicked]}>
        <privateMessageContext.Provider value={[message, setMessage]}>
            {invitePrivateButtonClicked ? <Invite darkMode={props.darkMode} group={clickOnPrivateItem} type="private" />
                :
                (<>
                    <div className="container " ref={containerRef} style={{ maxHeight: "500px", overflowY: "auto", scrollBehavior: 'smooth' }}>
                        <div className="row bg-primary border  flex-1 mb-3" style={{ borderRadius: "20px", height: "80px", marginLeft: "10px", marginRight: "10px", position: "sticky", top: 0 }}>
                          ...
                          { foundGroup && <div className="col-md-2 mt-1 mb-1" style={{ marginLeft: "1rem" }}>
                            ...
                            </div>}
                            {
                                user.role !== 1 && user.role !== 2 ? null
                                    : (<div className="col-md-1 offset-md-7 float-end mt-4">
                                        <button className="btn btn-success" onClick={e => setInvitePrivateButtonClicked(true)}>Invite</button>
                                    </div>)
                                        ...
                                        <div className={`row col-8 mt-3 mb-2  ${message.sender === user.id ? '' : 'offset-2 float-end '}`} key={index + 1} >
                                            ...
                                            {senderInfos ?
                                                <div className={`col-1 me-3 ${user.id === 1 ? null : message.sender === user.id ? '' : 'offset-1'} `}>
                                                    <img alt="img" src={`http://localhost/${senderInfos['employee']['image']}`} width={50} height={50} className="rounded-circle" />
                                                </div>
                                                : null
                                            }
                                                        ...
                                                        message.type === 'application/pdf' || message.type === 'application/msword' || message.type === 'application/vnd.ms-excel'
                                                            ...
                                                            : message.type === 'video/mp4' || message.type === 'video/webm'
                                                                ...
                                                                : message.type === "image/jpeg" || message.type === "image/png" || message.type === "image/gif"
                                                                    ? <img alt="img"  src={`http://localhost/${message.path}`} width="100%" height="300" style={{ borderRadius: '25px' }} />
                    ...
                    <WriteMessage handleMessageSubmit={handleMessageSubmit} type="private" group={clickOnPrivateItem} />

                </>)
            }
        </privateMessageContext.Provider>
    </invitePrivateButtonClickedContext.Provider>
    );
}
